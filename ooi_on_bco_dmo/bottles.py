...
"""
@author Andrew Reed
@brief Provides methods for working with OOI Discrete Water Sampling Data
"""
...
def clean_data(bottle_data: pd.DataFrame) -> pd.DataFrame:
    ...
    NOTE: If working with BCO-DMO, will need to update the dataframe
    column names to match the BCO-DMO naming schema.
    ...
    bottle_data["Start_Time"] = bottle_data["Start_Time"].apply(lambda x: convert_times(x))
    bottle_data["CTD_Bottle_Closure_Time"] = bottle_data["CTD_Bottle_Closure_Time"].apply(lambda x: convert_times(x))
